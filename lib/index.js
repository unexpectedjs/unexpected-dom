...
module.exports = {
  ...
  installInto: function (expect) {
    ...
    expect.addAssertion('HTMLElement', 'to satisfy', function (expect, subject, value) {
      var flags = this.flags;
      var promiseByKey = {
        nodeName: expect.promise(function () {
          if (value && typeof value.nodeName !== 'undefined') {
            return expect(subject.nodeName.toLowerCase(), 'to satisfy', value.nodeName);
          }
        }),
        attributes: {}
      };
        ...
        return expect.promise.settle(promiseByKey).then(function () {
          expect.fail({
            diff: function (output, diff, inspect, equal) {
              output
                .prismPunctuation('<')
                .prismTag(subject.nodeName.toLowerCase());
              var canContinueLine = true;
              if (promiseByKey.nodeName.isRejected()) {
                var nodeNameError = promiseByKey.nodeName.reason();
                output.sp().annotationBlock(function () {
                  this
                    .error((nodeNameError && nodeNameError.label) || 'should satisfy') // v8: err.getLabel()
                    .sp()
                    .append(inspect(value.nodeName));
                }).nl();
                canContinueLine = false;
              }
              Object.keys(attrs).forEach(function (attributeName) {
                ...
                if ((promise && promise.isFulfilled()) || (!promise && (!onlyAttributes || expectedAttributeNames.indexOf(lowerCaseAttributeName) !== -1))) {
                  canContinueLine = true;
                } else {
                  output
                    .sp()
