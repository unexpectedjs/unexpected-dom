...
function getCanonicalAttributes(element) {
  ...
  Object.keys(attrs).sort().forEach(function (key) {
    if (key === 'class' && Array.isArray(attrs[key])) {
      result[key] = attrs[key].sort();
    } else {
  ...
  installInto: function (expect) {
    ...
    expect.addAssertion('HTMLElement', 'to [only] have (attribute|attributes)', function (expect, subject, cmp) {
      ...
      if (Array.isArray(cmp)) {
        ...
        if (cmp['class'] && typeof cmp['class'] === 'string') {
          cmp['class'] = cmp['class'].split(' ');
        }
        ...
        if (this.flags.exhaustively) {
          ...
          if (cmp['class']) {
            expect(attrs['class'], 'to be an', 'array');

            var classes = cmp['class'];
            console.log(attrs['class'], 'to contain', classes);

            // classes.forEach(function (className) {
            //   expect(attrs['class'], 'to contain', className);
            // });

            // delete attrs['class'];
            // delete cmp['class'];
            cmp['class'] = expect.it.bind(this, 'to contain').apply(classes);
          }

          console.log(cmp['class']._expectations);
